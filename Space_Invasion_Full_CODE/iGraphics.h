...
void iKeyUpboard(unsigned char);
void iSpecialUpKeyboard(unsigned char);
...
void keyboardUpHandler1FF(unsigned char key, int x, int y)
{
	iKeyUpboard(key);
	glutPostRedisplay();
}
...
void keyboardUpHandler2FF(int key, int x, int y)
{
	iSpecialUpKeyboard(key);
	glutPostRedisplay();
}
...
void iInitialize(int width=500, int height=500, char *title="iGraphics")
{
	...
	glutKeyboardUpFunc(keyboardUpHandler1FF);

 	glutSpecialFunc(keyboardHandler2FF); //special keys
 	glutSpecialUpFunc(keyboardUpHandler2FF);
...
void iRotationStart(int x, int y,double angle)
{
		glPushMatrix();
		glTranslatef(x,y,0);
...
void iRotationStop(int x,int y){
